...
# "japanize_matplotlib"は，matplotlibのグラフ上に日本語フォントを表示させるライブラリ
# $ pip install japanize_matplotlib
# でインストール可能．実行環境ですでにmatplotlibのグラフ上に日本語フォントを表示できるならこれは不要．
# japanize_matplotlibをインストールできない，または，matplotlibで日本語を表示できない場合は，
# 下のクラスメソッドでmatplotlib.pyplotでの描画をする際のtitleやlabelから日本語を消す必要がある

# グラフ描画
def make_figure(X,Y,titlename='',Xlabel='',Ylabel='',label='',plot_type='scatter',point_size=5,pcolor='tab:blue',lcolor='tab:blue',lwidth=1.0,fig_size=(8,7),Xlog=False,Ylog=False,Yerr=None,consecutive=None,filename=''): 
...
class Ising2():
    def __init__(self,Ny,Nx,J,H,dir_name,max_kBT=6,N_kBT=200,N_MCS=40000,sampling_period=1,equilibrium_num=2,Q=500,cold=True):
        ...
        def Hamiltonian(spin_table):
            def closest_spin_pair(spins): # 最近隣接スピン対<i,j>について,Si*Sjを足し上げる．また，周期境界条件を課した
                # スピン行列を1つ上にずらしたものと1つ左にずらしたものを足して
                # 元の行列とアダマール積(同じ位置にある要素同士の積で行列積ではない)をとると，最近隣接スピン対<i,j>についてSi*Sjを足し上げたものが出てくる
                # Pythonではfor文より行列操作の方が圧倒的に速い(サイズにもよるが数十倍程度も違う)のでこの方式を採用
                up = np.vstack((spins[1:Ny,:],np.reshape(spins[0,:], (1, Nx))))
                left = np.hstack((spins[:,1:Nx],np.reshape(spins[:,0], (Ny, 1))))
                return np.sum(spins*(up+left))
        ...
        self.N_kBT = N_kBT # (0,max_kBT]の分割数(刻み)
        ...
        min_exp = -4
        steps_under_kBTc = int(N_kBT/(np.log(self.kBTc)-min_exp+np.log(self.max_kBT-self.kBTc)-min_exp)*(np.log(self.kBTc)-min_exp))
        ...
        self.kBT_list = np.concatenate([self.kBTc-np.logspace(np.log(self.kBTc-0.0001),min_exp,num=steps_under_kBTc,base=np.e), [self.kBTc], self.kBTc+np.logspace(min_exp,np.log(self.max_kBT-self.kBTc),num=self.N_kBT-steps_under_kBTc-1,base=np.e)])
    ...
    def MonteCarlo(self,MCmode,is_save=True): 
        ...
        class UnionFind:
            ...
            def root(self, x):
                ...
                if self.parent[x] < 0:
                    # 自分が根
                    return x
                # 要素xの親を要素xの根に付け替えることで次の呼び出しが早くなる
                self.parent[x] = self.root(self.parent[x])
                return self.parent[x]
            def size(self, x):
                # 要素xの所属する連結集合のサイズを調べる
        ...
        def SwendsenWang_algorithm(s_before, e_2JkBT, E_kBT): 
            # Swendsen-Wang アルゴリズムは，全てのスピンをクラスタリングして
            # 各クラスターに属するスピンを確率的にひっくり返すことを繰り返す
            dire = ((1, 0), (0, 1))
            s_trial = s_before.copy()
        ...
        if is_save: # データの保存
            prefix = 'Ising,'+str(self.MCmode)+','+str(self.Nx)+','+str(self.Ny)+','+str(self.J)+','+str(self.H)+','+str(self.max_kBT)+','+str(self.N_kBT)+','+str(self.N_MCS)+','+str(self.equilibrium_num)+','+str(self.Q)+','+self.start
    ...
    def visualize_temperature_development(self):
        ...
        title = r"磁化率 $\chi$"
        Ylabel = r"1スピンあたりの"+title
        #make_figure(np.log(self.X_history),"磁化率の対数log(χ)",filename=make_filename('4')
    ...
    def reconstructor(self,filename):
        self.MCmode,self.Nx,self.Ny,self.J,self.H,self.max_kBT,self.N_kBT,self.N_MCS,self.equilibrium_num,self.Q,self.start = filename.split(',')[1:]
        file_path = self.dir_path+self.MCmode+'/'+filename
        npz_data = np.load(file_path)
        self.Nx, self.Ny, self.N_kBT, self.N_MCS, self.Q = map(int,[self.Nx, self.Ny,self.N_kBT,self.N_MCS,self.Q])
        self.J, self.H, self.max_kBT, self.N_kBT, self.equilibrium_num = map(float,[self.J,self.H,self.max_kBT,self.N_kBT,self.equilibrium_num])
    ...
    def calculate_critical_index(self): # 臨界指数の計算
        ...
        # なお，スケーリング則によれば，alpha+2beta+gamma = 2が成り立つ
        """
        ...
        # Tc近傍の点のうちどこからどこまでをを近似に使用するか(alphaとgammaについては有限系であることが効いてくる)
        start_effective_point = 5
        end_effective_point = 15
        ...
        a, b = np.polyfit(X_alpha[start_effective_point:end_effective_point],Y_alpha[start_effective_point:end_effective_point],1)
        make_figure(X_alpha,a*X_alpha+b,plot_type='plot',lcolor='black',label=r"$T_c$近傍({}-{})点による近似直線".format(end_effective_point,start_effective_point)+'\n'+r"($C/k_B$={:.3f}$\log(|T-T_c|/T_c)+${:.3f})".format(a,b),consecutive=1,fig_size=figsize)
        ...
        alpha, b = np.polyfit(X_alpha[start_effective_point:end_effective_point],Y_alpha[start_effective_point:end_effective_point],1)
        make_figure(X_alpha,alpha*X_alpha+b,plot_type='plot',lcolor='black',label=r"$T_c$近傍({}-{})点による近似直線($\alpha={:.3f}$)".format(end_effective_point,start_effective_point,-alpha),consecutive=1,fig_size=figsize)
        ...
        gamma, b = np.polyfit(X_gamma[start_effective_point:end_effective_point],Y_gamma[start_effective_point:end_effective_point],1)
        make_figure(X_gamma,gamma*X_gamma+b,plot_type='plot',lcolor='black',label=r"$T_c$近傍({}-{})点による近似直線($\gamma={:.3f}$)".format(end_effective_point,start_effective_point,-gamma),consecutive=1,fig_size=figsize)
...
def main():
    # 理論計算による相転移温度TcはH=0で，kBTc=2J/ln(1+√2)=2.2691853J
    ...
    I = Ising2(Ny,Nx,J,H,dir_name=dir_name,max_kBT=max_kBT,N_kBT=N_kBT,N_MCS=N_MCS,sampling_period=sampling_period,equilibrium_num=equilibrium_num,Q=Q,cold=cold) # coldstartがデフォルト
